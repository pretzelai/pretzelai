...
from pretzelai.coreconfig import CoreConfig
from pretzelai.jlpmapp import HERE, YARN_PATH
from pretzelai.semver import Range, gt, gte, lt, lte, make_semver
...
class _AppHandler:
    ...
    def link_package(self, path):
        ...
        self.logger.warning(
            "Installing %s as a linked package because it does not have extension metadata:",
            path,
        )
    ...
    def _format_no_compatible_package_version(self, name):
        ...
        if lab_newer_than_latest:
            ...
            return (
                'The extension "%s" does not yet support the current version of '
                "JupyterLab.\n" % name
            )

        parts = [
            "No version of {extension} could be found that is compatible with "
